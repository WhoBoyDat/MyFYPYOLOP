...
def process_segmentation(da_seg_mask, ll_seg_mask, img_det):
    ...
    bottom_row = resized_mask[int(h1 * y_axis), :]  # Bottom row of the lane mask
    lane_pixels = np.where(bottom_row > 0)[0]
    top_row = resized_mask[int(h1 * 0.68), :]
    ...
    if len(lane_pixels) >= 2:
        ...
        cv2.circle(img_det,(top_right, 489), 5 ,(255 ,0 ,255), -1)
        cv2.circle(img_det,(top_left, 489), 5 ,(255 ,0 ,255), -1)
        cv2.circle(img_det,(top_center, 489), 5 ,(255 ,0 ,255), -1)
        cv2.circle(img_det,(bot_right, 545), 5 ,(255 ,0 ,0), -1)
        cv2.circle(img_det,(bot_left, 545), 5 ,(0 ,0 ,255), -1)
        cv2.circle(img_det,(bot_center, 545), 5 ,(255 ,0 ,255), -1)
...
def detect(cfg, opt):
    ...
    for path, img, img_det, vid_cap, shapes in dataset:
        ...
        if not detected:
            ...
            if steering_angle >= 25 and not lane_position == "left": #Steer Right Limit
                pwm.value = True
                steering.value = True
                if pot_value is not None:
                    print(f"No Steering: Potentiometer Value: {int(steering_angle)}")

            if steering_angle <= -25 and not lane_position == "right": #Steer Left Limit
                pwm.value = True
                steering.value = True
                if pot_value is not None:
                    print(f"No Steering: Potentiometer Value: {int(steering_angle)}")
            ...
            if lane_position == "center" or not lane_position: # Joystick IDLE
                if 1 <= steering_angle <= 30:
                    print("Drifting Left, turning right")
                    pwm.value = False
                    steering.value = True

                if -30 <= steering_angle <= -1:
                    print("Drifting right, turning Left")
                    pwm.value = False
                    steering.value = False
